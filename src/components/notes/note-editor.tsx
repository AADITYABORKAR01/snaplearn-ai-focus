...
export function NoteEditor({
  ...
  const insertTimestamp = () => {
    if (textareaRef.current) {
      ...
      const newNotes = notes.substring(0, cursorPosition) + timestampText + notes.substring(cursorPosition);
      setNotes(newNotes);
  ...
  return <div className="border rounded-lg bg-card p-4 h-full flex flex-col">
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-medium text-lg">Notes</h3>
        <div className="flex space-x-2">
          <Button variant="outline" size="sm" onClick={toggleRecording} className={isRecording ? "text-red-500" : ""}>
      ...
      <Tabs defaultValue="editor" className="flex-1 flex flex-col">
        ...
        <TabsContent value="editor" className="flex-1 flex flex-col mt-4 space-y-4">
          <Textarea ref={textareaRef} value={notes} onChange={e => setNotes(e.target.value)} placeholder="Start taking notes here..." className="flex-1 resize-none min-h-[200px]" />
          <div className="flex justify-end space-x-2">
            <Button variant="outline" onClick={handleDownloadNotes} disabled={!notes.trim()}>
              <Download size={16} className="mr-1" />
              Download
            </Button>
            <Button onClick={handleSaveNotes} disabled={!notes.trim()} className="bg-snapblue hover:bg-snapblue-dark">
        ...
        <TabsContent value="timestamps" className="flex-1 mt-4">
          {timestamps.length > 0 ? <div className="space-y-2">
              {timestamps.map((stamp, index) => <div key={index} className="p-2 border rounded hover:bg-muted/50 cursor-pointer flex items-start">
